...
public class TabularLabelContainer extends LabelContainer {
  ...
  public TabularLabelContainer(URL labelUrl, VolumeContainerSimple volume, Dictionary dictionary) {
  ...
  public List<PointerStatement> getTabularPointers() {
    ...
    List<PointerStatement> tabularPointers = new ArrayList<>();
  ...
  public List<ObjectStatement> getAllObjects(final DictIdentifier identifier) {
    ...
    List<ObjectStatement> objectStatements = new ArrayList<>();
    List<ObjectStatement> innerLevelStatements = new ArrayList<>();
  ...
  public TabularData getTabularData(final String tableType, long numRows) {
    ...
    if (foundObjects.size() == 1) {
      ...
      if (tabFileUrl != null) {
        if (tableObj.getIdentifier().toString().equals("SPREADSHEET")) {
          String delimiterName = tableObj.getAttribute("FIELD_DELIMITER").getValue().toString();
