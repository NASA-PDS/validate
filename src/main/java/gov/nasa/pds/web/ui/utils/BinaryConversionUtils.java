...
public class BinaryConversionUtils {
  ...
  /**
   * 
   * Converts an array of bytes into an int. The array of bytes may be of length 1, 2, 3, or 4, in
   * big-endian order. That is, b[0] holds the most significant bits, b[n-1] the least. The bytes
   * are assumed to be signed.
   * 
   * @param b the array of bytes, in big-endian order
   * @param n the number of bytes to convert in the array, from 1 to the size of an int
   ...
   * 
   * Converts an array of bytes into an int. The array of bytes may be of length 1, 2, 3, or 4, in
   * little-endian order. That is, b[n-1] holds the most significant bits, b[n-1] the least. The
   * bytes are assumed to be signed.
   * 
   * @param b the array of bytes, in little-endian order
   * @param n the number of bytes to convert in the array, from 1 to the size of an int
   * @return the signed int value represented by the bytes
   */
  ...
  /**
   * 
   * Converts an array of bytes into a long, permitting unsigned bytes. The array of bytes may be of
   * length 1, 2, 3, or 4, in big-endian order. That is, b[0] holds the most significant bits,
   * b[n-1] the least.
   * 
   * @param b the array of bytes, in big-endian order
   * @param n the number of bytes to convert in the array, from 1 to the size of an int
  ...
  public static long convertMSBUnsigned(int[] b, int n) {
    ...
    if (n == 1) {
      anUnsignedInt = 0xFF & (b[0]);
    }

    if (n == 2) {
      anUnsignedInt = (0xFF & (b[0])) << 8 | (0xFF & (b[1]));

    }
    if (n == 3) {
      anUnsignedInt =
          ((0xFF & (b[0])) << 16 | (0xFF & (b[1])) << 8 | (0xFF & (b[2]))) & 0xFFFFFFFFL;
    }
    if (n == 4) {
      anUnsignedInt =
          ((0xFF & (b[0])) << 24 | (0xFF & (b[1])) << 16 | (0xFF & (b[2])) << 8 | (0xFF & (b[3])))
              & 0xFFFFFFFFL;
    }
  ...
  public static long convertLSBUnsigned(int[] b, int n) {
    long anUnsignedInt = 0;
    if (n == 1) {
      anUnsignedInt = 0xFF & (b[n - 1]);
    }
    if (n == 2) {
      anUnsignedInt = (0xFF & (b[1])) << 8 | (0xFF & (b[0]));
    }
    if (n == 3) {
      anUnsignedInt =
          ((0xFF & (b[2])) << 16 | (0xFF & (b[1])) << 8 | (0xFF & (b[0]))) & 0xFFFFFFFFL;
    }
    if (n == 4) {
      anUnsignedInt =
          ((0xFF & (b[3])) << 24 | (0xFF & (b[2])) << 16 | (0xFF & (b[1])) << 8 | (0xFF & (b[0])))
              & 0xFFFFFFFFL;
    }
    return anUnsignedInt;
  }
  ...
  /**
   * Convert byte array to String. Use the correct encoding, ASCII (7-bit) or EBCDIC (8-bit) In the
   * case it is not able to use the correct encoding, display the bytes in decimal format as
   * follows: [87,87,....]
   * 
   * @param byteArray
   * @return
  ...
  /**
   * Appends a logical 1 or 0 to the existing string based on the value of the bit Used to convert a
  ...
  public static long convertByteArrayToLong(byte[] b) {
    ...
    aSignedLong = ((long) (0xFF & b[0]) << 56) | ((long) (0xFF & b[1]) << 48)
        | ((long) (0xFF & b[2]) << 40) | ((long) (0xFF & b[3]) << 32) | ((long) (0xFF & b[4]) << 24)
   ...
   * @return Byte array in integer format with all the bits ordered in IEEE-754 4 byte format
   */
  public static int vaxFTypeToIEEESingle(byte[] b) {
    ...
    int sign = (b[1] & 0x00000080) << 24;
    int e1 = (b[1] & 0x0000007F) << 1;
    int e0 = (b[0] & 0x00000080) >>> 7;
    ...
    // Mantissa extraction
    int m0 = (b[0] & 0x0000007F) << 16;
    int m1 = (b[3] & 0x000000FF) << 8;
    int m2 = (b[2] & 0x000000FF);
  ...
  /**
   * Convert from VAX D-type to IEEE double precision. For compatibility, the mantissa is truncated
   * from 55 bits to 52 bits
   *
   * @param b - byte array of size 8 bytes
   * @return Byte array in long format with all the bits ordered in IEEE-754 8 byte format
   */
  public static long vaxDTypeToIEEEDouble(byte[] b) {
    ...
    long sign = b[1] & (0x0000000000000080L) << 56;

    long e1 = (b[1] & 0x000000000000007FL) << 1;
    long e0 = (b[0] & 0x0000000000000080L) >>> 7;
    ...
    long m0 = (b[0] & 0x000000000000007FL) << 48;
    long m1 = (b[3] & 0x00000000000000FFL) << 40;
    long m2 = (b[2] & 0x00000000000000FFL) << 32;
    long m3 = (b[5] & 0x00000000000000FFL) << 24;
    long m4 = (b[4] & 0x00000000000000FFL) << 16;
    long m5 = (b[7] & 0x00000000000000FFL) << 8;
    long m6 = (b[6] & 0x00000000000000FFL);
  ...
  public static long vaxGTypeToIEEEDouble(byte[] b) {
    ...
    long e1 = (b[1] & 0x000000000000007FL) << 4;
    long e0 = (b[0] & 0x00000000000000F0L) >>> 4;
    ...
    // Mantissa extraction
    long m0 = (b[0] & 0x000000000000000FL) << 48;
    long m1 = (b[3] & 0x00000000000000FFL) << 40;
    long m2 = (b[2] & 0x00000000000000FFL) << 32;
    long m3 = (b[5] & 0x00000000000000FFL) << 24;
    long m4 = (b[4] & 0x00000000000000FFL) << 16;
  ...
  public static int pcReal4BTypeToIEEESingle(byte[] b) {
    ...
    int sign = (b[3] & 0x00000080) << 24;
    int e1 = (b[3] & 0x0000007F) << 1;
    int e0 = (b[2] & 0x00000080) >>> 7;
    ...
    // Mantissa extraction
    int m0 = (b[2] & 0x0000007F) << 16;
    int m1 = (b[1] & 0x000000FF) << 8;
    int m2 = (b[0] & 0x000000FF);
  ...
  public static long pcReal8BTypeToIEEEDouble(byte[] b) {
    ...
    long sign = b[7] & (0x0000000000000080L) << 56;

    long e1 = (b[7] & 0x000000000000007FL) << 4;
    long e0 = (b[6] & 0x00000000000000F0L) >>> 4;
    ...
    // Mantissa extraction
    long m0 = (b[6] & 0x000000000000000FL) << 48;
    long m1 = (b[5] & 0x00000000000000FFL) << 40;
    long m2 = (b[4] & 0x00000000000000FFL) << 32;
    long m3 = (b[3] & 0x00000000000000FFL) << 24;
    long m4 = (b[2] & 0x00000000000000FFL) << 16;
    long m5 = (b[1] & 0x00000000000000FFL) << 8;
    long m6 = (b[0] & 0x00000000000000FFL);
