...
public class Comparators {
  ...
  public final static Comparator<Column> TABULAR_COLUMN_COMPARATOR = new Comparator<Column>() {
    ...
    public int compare(Column left, Column right) {
      return NUMERIC_COMPARATOR.compare(left.getIndex(), right.getIndex());
    }
  };
  ...
  public final static Comparator<Column> TABULAR_COLUMN_NAME_COMPARATOR = new Comparator<Column>() {
    @Override
    public int compare(Column left, Column right) {
      return CASE_INSENSITIVE_COMPARATOR.compare(left.getName(), right.getName());
    }
  };

  public final static Comparator<Column> TABULAR_COLUMN_DATATYPE_COMPARATOR =
      new Comparator<Column>() {
        @Override
        public int compare(Column left, Column right) {
          return CASE_INSENSITIVE_COMPARATOR.compare(left.getDataType(), right.getDataType());
        }
      };
  ...
  public final static Comparator<NewValue> NEW_VALUE_COMPARATOR = new Comparator<NewValue>() {
    ...
    public int compare(NewValue left, NewValue right) {
      return CASE_INSENSITIVE_COMPARATOR.compare(left.getKey(), right.getKey());
    }
  };

  public final static Comparator<VolumeContainer> VOLUME_CONTAINER_COMPARATOR =
      new Comparator<VolumeContainer>() {
        @Override
        public int compare(final VolumeContainer left, final VolumeContainer right) {
          return CASE_INSENSITIVE_COMPARATOR.compare(left.getVolId(), right.getVolId());
        }
      };

  public final static Comparator<ExceptionContainer> EXCEPTION_CONTAINER_COMPARATOR =
      new Comparator<ExceptionContainer>() {
        @Override
        public int compare(final ExceptionContainer left, final ExceptionContainer right) {
