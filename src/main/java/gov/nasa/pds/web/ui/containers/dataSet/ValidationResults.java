...
public class ValidationResults implements Serializable, BaseContainerInterface {
  ...
  public static final transient String GET_PROBLEMS =
      "SELECT problem FROM `%s` WHERE errortype = '%s' LIMIT %s OFFSET %s"; //$NON-NLS-1$

  public static final transient String GET_FILE_PROBLEMS =
      "SELECT problem FROM `%s` WHERE path = '%s' LIMIT %s"; //$NON-NLS-1$
  ...
  private final Map<ProblemType, Integer> groupCount = new HashMap<>();
  ...
  public boolean addProblem(final File file, final String key, final ProblemType type,
    ...
    final SimpleProblem problem =
        new SimpleProblem(file, this.volume, key, type, lineNumber, arguments);
  ...
  public static class SimpleProblem implements Serializable {
    ...
    public SimpleProblem(final File file, final File root, final String key, final ProblemType type,
        final Integer lineNumber, final Object... arguments) {
      for (Object obj : arguments) {
        if (obj != null && !(obj instanceof java.io.Serializable)) {
          throw new RuntimeException(
              "Unable to serialize object of type " + obj.getClass().getName() + " in file "
                  + file.toString() + " on line " + lineNumber);
        }
      }
      this.fileObj = file;
  ...
  public static class NewValue implements Serializable {
    ...
    public NewValue(final File file, LabelParserException problem, final File root) {
      this.fileObj = file;
      this.file = new FileMirror(file, root);
    ...
    public boolean equals(Object obj) {
      ...
      return newVal.getKey().equals(this.key) && newVal.getValue().equals(this.value);
    }
