...
/**
 * The base class for validation rules. To implement validation rules, write the
 * validation tests as public void methods and annotate them with the
 * {@link ValidationTest} annotation. The tests will be invoked in they appear
 * within the class.
 */
public abstract class AbstractValidationRule implements ValidationRule {
     ...
     * Validation tests are denoted by public methods annotated with the
     * {@link ValidationTest} annotation.
     *
     * @param theContext
     *            the context for the rule, which must be a {@link RuleContext}
     * @throws {@inheritDoc}
     */
    @Override
    ...
    /**
     * Gets a rule context for validating a file or directory inside the current
     * target.
     *
     * @param child
     *            the child target
    ...
    protected RuleContext getChildContext(URL child) throws MalformedURLException, URISyntaxException {
        ...
        newContext.setValidContext(context.getValidContext());
     ...
     * @param targetFile
     *            the validation target file containing the problem
     * @param lineNumber
     *            the line number, or -1 if no line number applies
     * @param columnNumber
    ...
    protected void reportError(ProblemDefinition defn, URL targetUrl, int lineNumber, int columnNumber) {
        ValidationProblem problem = new ValidationProblem(defn, new ValidationTarget(targetUrl), lineNumber,
                columnNumber, defn.getMessage());
     ...
     *            the problem definition
     * @param target
     *            the validation target containing the problem
     * @param lineNumber
     *            the line number, or -1 if no line number applies
     * @param columnNumber
     *            the column number, or -1 if no column number applies
    ...
    protected void reportError(ProblemDefinition defn, URL target, int lineNumber, int columnNumber, String message) {
        ValidationProblem problem = new ValidationProblem(defn, new ValidationTarget(target), lineNumber, columnNumber,
                message);
        listener.addProblem(problem);
    }
