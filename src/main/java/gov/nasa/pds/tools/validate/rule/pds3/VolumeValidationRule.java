...
public class VolumeValidationRule extends AbstractValidationRule implements Observer {
  ...
  private static final Pattern VOLUME_README_PATTERN =
      Pattern.compile("aareadme.txt", Pattern.CASE_INSENSITIVE);

  /**
   * The process ID for the validation process. This is an arbitrary constant.
   */
  ...
  public void validateVolume() throws Exception {
    ...
    String dictionaryPath =
        SettingsManager.INSTANCE.getString(ValidationSettings.PDS3_DICTIONARY_PATH, null);
    ...
    DataSetValidator validator =
        new DataSetValidator(PROC_ID, FileUtils.toFile(getTarget()), status);
   ...
   * @param object object to use in update
   */
  @Override
  public void update(Observable validator, Object object) {
    if (object instanceof Bucket) {
      ...
      String message = this.status.getMessageKey() + " (step " + this.status.getStep() + ")";
      LOG.info(message);
      this.status.seen();
    }
  }
  ...
  private void postProblems(final Bucket bucket) {
    for (SimpleProblem problem : bucket.getProblems()) {
      ...
      ProblemDefinition problemDef = new ProblemDefinition(exceptionType, ProblemType.INVALID_LABEL,
          "File or label has errors", Standard.PDS3_STANDARDS_REFERENCE, null);
  ...
  /**
   * Formats an error message based on a key to a message template and arguments to be inserted into
   * the message.
   * 
