...
public class FileAndDirectoryNamingRule extends AbstractValidationRule {
  ...
  protected static final Pattern NAMING_PATTERN = Pattern.compile("[A-Za-z0-9][A-Za-z0-9_.-]*");
    ...
    // Protected function to allow this and children function to check whether a given filename is prohibited or not.
    protected boolean isFilenameProhibited(String filename){
        ...
        LOG.debug("isFilenameProhibited:filename,filenameIsProhibitedFlag {},{}",filename,filenameIsProhibitedFlag);
        return(filenameIsProhibitedFlag);
    }

    // Protected function to allow this and children function to check whether a given filename is too long or not.
    protected boolean isFilenameTooLong(String filename){
        ...
        LOG.debug("isFilenameTooLong:filename,filenameIsTooLongFlag {},{}",filename,filenameIsTooLongFlag);
        return(filenameIsTooLongFlag);
    }

    // Protected function to allow this and children function to check whether a given filename contains only valid characters.
    protected boolean isFilenameContainingLegalCharacters(String filename){
        ...
        LOG.debug("isFilenameContainingLegalCharacters:filename,filenameContainsLegalCharactersFlag {},{}",filename,filenameContainsLegalCharactersFlag);
        return(filenameContainsLegalCharactersFlag);
    }

    // Protected function to allow this and children function to check whether a given directory contains extension or not.
    protected boolean isDirectoryContainingInvalidCharacter(String filename, boolean isDirectory){
        ...
        LOG.debug("isDirectoryContainingInvalidCharacter:filename,isDirectory,filenameHasExtensionFlag {},{},{}",filename,isDirectory,filenameHasExtensionFlag);
    ...
    if (isFilenameTooLong(name)) {
      reportError(
        ...
        // Note: The check (PROHIBITED_BASE_NAMES.contains(baseName) is too strict and commented 
        ...
        //LOG.debug("checkFileAndDirectoryNaming:name,baseName,PROHIBITED_BASE_NAMES.contains(baseName) {},{},{}",name,baseName,PROHIBITED_BASE_NAMES.contains(baseName));
        //LOG.debug("checkFileAndDirectoryNaming:name,PROHIBITED_BASE_NAMES.contains(name) {},{},{}",name,PROHIBITED_BASE_NAMES.contains(name));
