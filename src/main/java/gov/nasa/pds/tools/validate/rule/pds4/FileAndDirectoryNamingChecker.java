 ...
 * listener. This allow any class to call checkFileOrDirectoryNameWithChecker() to get a list of
 * ValidationProblem directly without a need to report errors to a listener.
 */
public class FileAndDirectoryNamingChecker extends FileAndDirectoryNamingRule {
  ...
  public List<ValidationProblem> checkFileAndDirectoryNamingWithChecker(List<Target> list) {
    ...
    for (Target t : list) {
      ValidationProblem validationProblem =
          checkFileOrDirectoryNameWithChecker(t.getUrl(), seenNames, t.isDir());
  ...
  private ValidationProblem checkFileOrDirectoryNameWithChecker(URL u,
      Map<String, String> seenNames, boolean isDirectory) {
    ...
    if (!isFilenameContainingLegalCharacters(name)) {
      ...
      return (this.constructError(isDirectory ? PDS4Problems.DIRECTORY_NAME_USES_INVALID_CHARACTER
    ...
    if (isDirectoryContainingInvalidCharacter(name, isDirectory)) {
      ...
      return (this.constructError(PDS4Problems.DIRECTORY_NAME_USES_INVALID_CHARACTER, u, -1, -1));
    }
    ...
    if (seenNames.containsKey(lcName)) {
      ...
      return (this.constructError(isDirectory ? PDS4Problems.DIRECTORY_NAME_CONFLICTS_IN_CASE
