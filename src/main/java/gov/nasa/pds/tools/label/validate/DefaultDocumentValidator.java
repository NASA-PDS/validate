  ...
  public boolean validate(ProblemHandler handler, Source xml) {
    ...
    if (xmlModels.isEmpty()) {
      handler.addProblem(new ValidationProblem(new ProblemDefinition(ExceptionType.WARNING,
          ProblemType.MISSING_SCHEMATRON_SPEC, "No schematron specification found in the label."),
          systemIdUrl));
    } else {
      ...
      for (TinyNodeImpl xmlModel : xmlModels) {
        ...
        if (matcher.matches()) {
          if (matcher.group(3) != null) {
            if (!VersionInfo.getSchematronNamespace().equals(matcher.group(3).trim())) {
              handler.addProblem(new ValidationProblem(
                  new ProblemDefinition(ExceptionType.WARNING, ProblemType.BAD_SCHEMATYPENS,
                      "Value of the 'schematypens' attribute, '" + matcher.group(3).trim() + "', "
                          + "in the schematron specification is not "
                          + "equal to the recommended value of '"
                          + VersionInfo.getSchematronNamespace() + "'."),
                  systemIdUrl, xmlModel.getLineNumber(), -1));
            }
          } else {
            handler.addProblem(new ValidationProblem(
                new ProblemDefinition(ExceptionType.WARNING, ProblemType.MISSING_SCHEMATYPENS,
