...
public class FileService {
  ...
  public static void printStackTraceToFile(String filename, Exception exception) {
    ...
    // Only attempt to write to stack trace if the user had enabled it or running in
    // debug mode.
    if (!FlagsUtil.getStackPrintingFlag()) {
      LOG.info(
          "Stack trace printing to an external file is an important tool for debugging but it is disabled.  Please use --enable-stack-printing  and re-run your command if desire to see a stack trace log.");
      return;
    }

    // Continue with stack trace printing.

    // Do a sanity check that the name should end with .log or .txt so as not to
    // overwrite a source code.
    if (!filename.endsWith(".txt") && !filename.endsWith(".log")) {
      LOG.error(
          "printStackTraceToFile:ERROR: This function can only write to file that ends with .txt or .log.  Name provided ["
              + filename + "]");
      System.out.println(
    ...
    // Print the stack strace to a temporary file and then concatenate to an
    ...
    try {
      ...
      if (existingFile.exists()) {
        ...
        // To not overwhelm the log file for a normal user, only print these messages if
        // in debug mode.
        if (LOG.isDebugEnabled()) {
          LOG.error("A stack trace has been appended to file " + filename + " EXCEPTION_TIME "
              + dtf.format(now));
          System.out.println(
              "FileService:printStackTraceToFile:ERROR: A stack trace has been appended to file "
                  + filename + " EXCEPTION_TIME " + dtf.format(now));
        }
      } else {
        ...
        // in debug mode.
        if (LOG.isDebugEnabled()) {
          LOG.error("A stack trace has been written to file " + filename + " EXCEPTION_TIME "
              + dtf.format(now));
          System.out.println(
              "FileService:printStackTraceToFile:ERROR: A stack trace has been written to file "
                  + filename + " EXCEPTION_TIME " + dtf.format(now));
        }
      }
    } catch (Exception tempException) {
      System.out
          .println("FileService:printStackTraceToFile: Cannot create PrintStream or write to file "
              + filename);
      System.exit(0);
    }
  }
  ...
  public static void printDocumentToFile(String filename, DOMSource domSource) {
    ...
    // Print the document to an external file. Note that this overwrite any existing
    // file.
