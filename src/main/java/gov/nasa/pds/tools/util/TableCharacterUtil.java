...
public class TableCharacterUtil  {
    ...
    private static String FIELD_LOCATION_SIMPLE  = "field_location";  // Used to retrieve individual field from node and not document.
    private static String FIELD_FORMAT_SIMPLE    = "field_format";    // Used to retrieve individual field from node and not document.
    private static String FIELD_LENGTH_SIMPLE    = "field_length";    // Used to retrieve individual field from node and not document.
    private static String DATA_TYPE_SIMPLE       = "data_type";       // Used to retrieve individual field from node and not document.

    private static List<String> fieldNumberList = new ArrayList<String>();
    private static List<String> fieldLocationList = new ArrayList<String>();
    private static List<String> fieldTypeList   = new ArrayList<String>();
    private static List<String> fieldLengthList = new ArrayList<String>();
    private static List<String> fieldFormatList = new ArrayList<String>();
  ...
  public void validateInBetweenFields(String record, int recordNumber) {
    ...
    for (int ii = 0; ii < fieldNumberList.size(); ii++) {
      ...
      // Check the gap between where the previous field end and the current field start for non blanks and report it.
      if (ii > 0) {
          LOG.debug("validateInBetweenFields:ii,previousEndIndex,startIndex {},{},{}",ii,previousEndIndex,startIndex);

          // Do a sanity check if the user has field_location of the current field before the end of the previous end index.
         ...
         String gapValue  = record.substring(previousEndIndex,startIndex).trim();  // Get the value between one field ends and the next field start.
         LOG.debug("validateInBetweenFields:ii,previousEndIndex,startIndex,gapValue {},{},{},[{}]",ii,previousEndIndex,startIndex,gapValue);
         if (gapValue.length() > 0) {
              if (gapValue.length() == 1 && gapValue.equals(",")) {
                  LOG.error("validateInBetweenFields:Values in between gap of field number (starts with 1) {} and {} is a comma:",fieldNumberList.get(ii-1),fieldNumberList.get(ii));
              } else {
